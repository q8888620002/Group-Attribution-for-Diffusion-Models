...
def main(args):
    ...
    for excluded_class in range(0, 10):
        ...
        for epoch in range(config["epochs"]):
            ...
            for j, (image, _) in enumerate(train_dataloader):
                ...
                if j % args.log_freq == 0:
                    print(
                        f"Epoch[{epoch+1}/{config['epochs']}],Step[{j}/{len(train_dataloader)}], loss:{loss.detach().cpu().item():.5f}, lr:{scheduler.get_last_lr()[0]:.6f}"
                    )
            ...
            if (epoch + 1) % 20 == 0 or global_steps == config["epochs"] * len(
                train_dataloader
            ):
                model_ema.eval()
                ...
                if config["dataset"] != "mnist":
                    ...
                    inception = InceptionV3([block_idx], normalize_input=False).to(
                        device
                    )
                ...
                os.makedirs(
                    f"results/{args.dataset}/retrain/samples/{excluded_class}",
                    exist_ok=True,
                )
                ...
                save_image(
                    samples,
                    f"results/{args.dataset}/retrain/samples/{excluded_class}/steps_{global_steps:0>8}.png",
                    nrow=int(math.sqrt(args.n_samples)),
                )
            ## Checkpoints for training

            if (epoch + 1) % (config["epochs"] // 2) == 0 or (epoch + 1) % config[
                "epochs"
            ] == 0:
                ...
                torch.save(
                    ckpt,
                    f"/projects/leelab/mingyulu/data_att/results/{args.dataset}/retrain/models/{excluded_class}/steps_{global_steps:0>8}.pt",
                )
        ...
        if config["dataset"] != "mnist":
            np.save(
                f"/projects/leelab/mingyulu/data_att/results/{args.dataset}/retrain/models/{excluded_class}/steps_{global_steps:0>8}.npy",
                np.array(fid_scores),
            )
