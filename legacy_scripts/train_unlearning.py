...
def main(args):
    ...
    for excluded_class in range(0, 10):
        ...
        max_steps_orcale_file = get_max_step_file(
            f"/projects/leelab/mingyulu/data_att/results/{args.dataset}/retrain/models/{excluded_class}/"
        )
        ...
        for epoch in range(args.epochs):
            ...
            for j, ((image_r1, _), (image_r2, _), (image_f, _)) in enumerate(
                zip(train_r1_dataloader, train_r2_dataloader, forget_dataloader)
            ):
                ...
                if args.loss_type == "type1":
                    ...
                    loss = alpha1 * loss_fn(eps_r, eps_r_frozen) - alpha2 * loss_fn(
                        eps_r, eps_f_frozen
                    )
                    ...
                    loss = alpha1 * loss_fn(eps_r, noise) + loss_fn(
                        eps_r, (eps_r_frozen - alpha2 * eps_f_frozen)
                    )
                ...
                if j % args.log_freq == 0:
                    print(
                        f"Epoch[{epoch+1}/{args.epochs}],Step[{j}/{len(train_r1_dataloader)}],loss:{loss.detach().cpu().item():.6f},lr:{scheduler.get_last_lr()[0]:.6f}"
                    )
            ...
            if (
                (epoch + 1) % (config["epochs"] // 10) == 0
                or (epoch + 1) % args.epochs == 0
                or (global_steps + 1) == config["epochs"] * len(train_r1_dataloader)
            ):
                ...
                os.makedirs(
                    f"results/{args.dataset}/unlearning/samples" + exp_settings,
                    exist_ok=True,
                )
                ...
                save_image(
                    samples,
                    f"results/{args.dataset}/unlearning/samples"
                    + exp_settings
                    + f"/steps_{global_steps:0>8}.png",
        ...
        torch.save(
            ckpt, path + "models" + exp_settings + f"/steps_{global_steps:0>8}.pt"
        )
