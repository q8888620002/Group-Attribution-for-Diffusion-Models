...
def main(args):
    ...
    for digit in range(11):
        ...
        for i in range(args.epochs):
            ...
            torch.save(ckpt, f"results/{digit}/steps_{global_steps:0>8}.pt")

            model_ema.eval()
            samples = model_ema.module.sampling(args.n_samples, clipped_reverse_diffusion=not args.no_clip, device=device)

            save_image(samples, f"results/{digit}steps_{global_steps:0>8}.png", nrow=int(math.sqrt(args.n_samples)))
