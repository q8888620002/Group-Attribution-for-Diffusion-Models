...
def parse_args():
    ...
    parser.add_argument(
        "--loss_type", type=str, help="define loss type", default="type4"
    )
...
def save_images(samples, directory, name_prefix):
    ...
    for i, img in enumerate(samples):
        ...
        save_image(img, os.path.join(directory, f"{name_prefix+i}.png"))
...
def main(args):
    ...
    results = {"FID": [], "clip_score_unlearn": [], "pred_probs": []}
    ...
    for target_digit in range(0, 10):
        ...
        model_unlearn_ema = ExponentialMovingAverage(
            model_unlearn, device=device, decay=1.0 - alpha
        )
        ...
        model_retrain_ema = ExponentialMovingAverage(
            model_retrain, device=device, decay=1.0 - alpha
        )
        ...
        if args.dataset == "mnist":
            ...
            print(
                f"Predicted probabilitys for {target_digit}, unlearn: {mean_pred_unlearn}; retrain: {mean_retrain}"
            )
            ...
            unlearn_dir = os.path.join(
                "/projects/leelab/mingyulu/data_att/results",
                args.dataset,
                "unlearning",
                "eval",
                str(target_digit),
            ...
            real_features = get_features(
                trainloader, mean, std, inception, args.n_samples, device
            )
            ...
            with torch.no_grad():
                for _ in range(n_batches):
                    ...
                    save_images(
                        torch.clamp((samples + 1.0) / 2, 0.0, 1.0),
                        unlearn_dir,
                        count * batch_size,
                    )
