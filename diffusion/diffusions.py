...
class DDPM(nn.Module):
    ...
    def forward(
        self, x: torch.tensor, noise: torch.tensor, t: torch.tensor = None
    ) -> torch.tensor:
        # x:NCHW
        ...
        for i in tqdm(range(self.timesteps - 1, -1, -1), desc="Sampling"):
            ...
            if target_class:
                x_t = self._reverse_diffusion_with_guidance(
                    x_t, t, noise, classifier, target_class, guidance_scale=1.0
                )
    ...
    @torch.no_grad()
    def _sampling(
        self, x_t, n_samples, clipped_reverse_diffusion=True, device="cuda"
    ) -> torch.tensor:
        """
        Sampling process for the model x_T -> x_0
        """
    ...
    def _reverse_diffusion_with_guidance(
        ...
        classifier_grad = self.compute_classifier_gradient(
            classifier, x_t, target_class
        )
        ...
        with torch.no_grad():
            ...
            x_0_pred = (
                torch.sqrt(1.0 / alpha_t_cumprod) * x_t
                - torch.sqrt(1.0 / alpha_t_cumprod - 1.0) * guided_pred_noise
            )
            x_0_pred.clamp_(-1.0, 1.0)
    ...
    def compute_classifier_gradient(
        ...
        target_prob = 1.0 - probabilities[:, target_class]
